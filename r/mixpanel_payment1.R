...
get_csv <- function(dataset) {
  for(i in dataset) {
    ...
    write.csv(events, paste0("~/r/mixpanel_payments/", counter, "__", first_day(month, year), ".csv"))

  }
}

get_csv_hk <- function(i) {
    month     <- i[1]
    year      <- i[2]
    ...
    events <- mixpanelGetEvents(
      ...
      daysPerBlock = 10,
      select = TRUE, # select = [ARRAY PROPERTIES]
      where = "\"HK\" in properties[\"mp_country_code\"]",
      verbose = TRUE
    )

    write.csv(events, paste0("~/r/mixpanel_payments/hk_payments_", month, "_", year, ".csv"))
}

setwd("~/r/temp")
get_csv_hk(c(02,2017))
