...
def export_clean_json file_name
  data = read_json_file "mixpanel/#{file_name}.json"
  export_json = []
  data.each do |el|
    _x = el["Longitude"].to_f
    _y = el["Latitude"].to_f
    export_json.push([_x,_y]) if _x != 0 && _y != 0
  end
  save_json_to_file "mixpanel/#{file_name}_positions_only.json", export_json
end

def print_length file_name
  data = read_json_file "mixpanel/#{file_name}.json"
  data_pos = read_json_file "mixpanel/#{file_name}_positions_only.json"
  p "----"
  p file_name
  p data.length
  p data_pos.length
